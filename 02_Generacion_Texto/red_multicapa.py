...
x_escalado = x_gen / np.abs(W1_max)
y_escalado =  y1 / np.abs(W1_max)
...
x = layers.Dense(8000, activation='gelu')(inputs)

x = layers.Dense(5000, activation='gelu')(x)

x = layers.Dense(2500, activation='gelu')(x)
...
multicapa_one_hot.compile(loss='mae', optimizer=keras.optimizers.Adam(0.0005), metrics=['mse','accuracy'])
